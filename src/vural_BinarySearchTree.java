 ...
 * Its construction promotes intuitive (and more importantly quick) searches on
 * the data structure.
 * 
 * Balancing the BST is beyond the scope of our course, but assuming then BST is
 * balanced, then finding
 * some element within the BST becomes more time efficient than a linear search,
 * it actually becomes O(log n).
 */
public class vural_BinarySearchTree implements BinarySearchTreeFunctions {
    private Node root;
    ...
    private void insertHelper(Node currentRoot, Node z) {
        if (z.getKey() > currentRoot.getKey()) {
            if (currentRoot.getRight() == null) {
                z.setParent(currentRoot);
                currentRoot.setRight(z);
            }
            else {
                insertHelper(currentRoot.getRight(), z);
            }
        }    	
    	else if (z.getKey() < currentRoot.getKey()) {
            if (currentRoot.getLeft() == null) {
                z.setParent(currentRoot);
                currentRoot.setLeft(z);
            }
            else {
                insertHelper(currentRoot.getLeft(), z);
            }
        }
    }
     ...
     * If a Node exists in the BST with the same key as Node z, then we should not
     * add z.
     * 
    ...
    public void insertNode(Node z) {
        ...
        else if ( this.getNode( this.root, z.getKey() ) != null)
    	{
    		return;
    	}
        else {
            insertHelper(this.root, z);
        }
    }

    /**
     * Updates an existing Node in the BST.
     * The specified Node should replace the node in the BST which has the same key
     * as Node z.
     * If there is no such node in the tree, then this update should be ignored.
     * 
     * Note: This is used primarily to update the held data within the node being
     * updated. The key value remains the same.
     * 
     * @param Node z - specifies the updated state of some Node that has already
     *             been inserted to the BST previously.
     */
    public void updateNode(Node z) {
    	
    	Node nodeToUpdate = getNode(this.root, z.getKey());
     ...
     * 3. Our right subtree
     * 
     * @param Node x - specifies which Node we are currently viewing as the relative
     *             root as we perform the pre-order traversal.
     */
     ...
     * When the pre-order traversal is complete, the list passed in will be fully
     * populated with information pertaining
     * to the order the in which the Nodes were visited.
     * 
     * @param Node                        x - specifies which Node we are currently
     *                                    visiting
     *                                    as the relative root as we perform the
     *                                    pre-order traversal.
     * @param java.util.ArrayList<String> list - a list of Strings that will be
     ...
     * @param Node                        x - specifies which Node we are currently
     *                                    visiting
     * @param String                      id - Can be used to distinguish some
     *                                    debugged data
     * @param java.util.ArrayList<String> result - a list of strings that will be
     *                                    populated based upon
     *                                    the order in which the Nodes are visited.
     */
     ...
     * 
     * @param Node x - specifies which Node we are currently viewing as the relative
     *             root as we perform the in-order traversal.
     */
     ...
     * When the in-order traversal is complete, the list passed in will be fully
     * populated with information pertaining
     * to the order the in which the Nodes were visited.
     * 
     * @param Node                        x - specifies which Node we are currently
     ...
     * 
     * @param Node x - specifies which Node we are currently viewing as the relative
     *             root as we perform the post-order traversal.
     */
     ...
     * When the post-order traversal is complete, the list passed in will be fully
     * populated with information pertaining
     * to the order the in which the Nodes were visited.
     * 
     * @param Node                        x - specifies which Node we are currently
     ...
     * 2. is contained within a subtree (in particular the right subtree) of the
     * specified Node
     * 
     * If the specified Node has no descendant with a larger key, return x itself.
     * 
     * @param Node x - specifies which Node we wish to start from while searching
     *             for his maximum descendant
     * 
     ...
     * 2. is contained within a subtree (in particular the left subtree) of the
     * specified Node
     * 
     * If the specified Node has no descendant with a smaller key, return x itself.
     * 
     * @param Node x - specifies which Node we wish to start from while searching
     *             for his minimum descendant
     * 
     ...
     * By successor, we mean some Node whose key value is the least upper bound of
     * the specified Node's key.
     * 
     ...
     * By predecessor, we mean some Node whose key value is the greatest lower bound
     * of the specified Node's key.
     * 
    ...
    /**
     * Attempts to find and return the Node, starting from x, which holds the
     * specified key.
     * 
     * If the specified key doesn't correspond to some Node that is a descendant of
     * x, return null.
     * 
     * @param Node x - specifies which Node we wish to start from while searching
     *             for the node
     * @param int  key - the key value indicating which Node we are attempting to
     *             find
     * 
     * @return the Node whose key value matches the key we specified (may be null if
     *         not found)
     */
    ...
    /**
     * Determines and returns the height of some subtree of the BST, starting from x
     * as the relative root Node.
     * 
     * Note: To determine the height of the entire BST, we would pass the actual
     * root of the entire BST to this method.
     * 
     * @param Node x - specifies which Node we wish to start from while determining
     *             the height.
     * 
     ...
     * 
     * By "replace", we mean that Node v is treated as if it resided where u was all
     * along. In other words:
     * 1. Parent: v's parent will be updated to point at what u's parent was
     * 2. Child: u's parent will treat v as its child, pointing to v where it used
     * to point to u.
     */
     ...
     * If the specified Node cannot be found in the BST, then this function should
     * not alter the BST.
     * Upon deleting the Node, is it required that the BST maintains the search
     * property and is connected after the deletion.
     * 
     * By search property, we mean that for any given Node in the BST:
     * 1. all his left descendants (those in his left subtree) have strictly smaller
     * key values
     * 2. all his right descendants (those in his right subtree) have strictly
     * greater key values
     * 
    ...
    public void deleteNode(Node z) {
    	...
    	//
    	if (this.getNode(this.root, z.getKey()) == null)
    	{
    		return;
    	}
    	...
    	if (z.getLeft() == null) // one or no child node
    	{
    		shiftNode(z, z.getRight());
    	}
    	else
    	{
    		if(z.getRight() == null) // one or no child node
    		{
